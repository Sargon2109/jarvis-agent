...
def build_system_prompt(
    ...
    return (
        ...
        "DELEGATE WHOLE JOBS, NOT STEPS. Each specialist you call starts with a "
        "blank context and sees only the instruction you give it. It cannot see "
        "what another specialist read, and their replies to you are short "
        "summaries, not the material itself. So never split 'read the rubric' "
        "and 'write the paper' into two calls — the writer would be working "
        "from a paraphrase of a document it never saw. Hand one specialist the "
        "whole job: read the sources AND produce the work.\n\n"
        "   When you know where the source material is — an attached file, a "
        "Canvas file, a course profile — put those paths in the instruction so "
        "the specialist opens the real thing rather than working from your "
        "description of it.\n\n"
